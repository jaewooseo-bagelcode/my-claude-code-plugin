...
// --- Coder output types (structured output from codex-coder) ---

/// Structured coder output matching the outputSchema for code implementation.
#[derive(Debug, Deserialize, Serialize)]
...
impl std::fmt::Display for CoderStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CoderStatus::Completed => write!(f, "completed"),
            CoderStatus::Partial => write!(f, "partial"),
            CoderStatus::Blocked => write!(f, "blocked"),
        }
    }
}

/// A single file change in the coder output.
#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
...
/// The outputSchema sent to codex to enforce structured coder output.
pub fn coder_output_schema() -> Value {
    serde_json::json!({
        "type": "object",
        "properties": {
        ...
        "required": ["status", "summary", "files_changed", "notes"],
        "additionalProperties": false
    })
}
