...
pub fn execute(
    ...
    // Security: ensure resolved path is within project (Rust canonicalize + starts_with)
    ...
    // Go-style: verify it's a regular file
    let metadata = fs::metadata(&canonical)
        .map_err(|e| format!("Failed to stat {}: {}", path, e))?;
    if !metadata.is_file() {
        return Err("Read: not a regular file".to_string());
    }
    ...
    // Calculate start/end based on parameters (support both offset/limit and start_line/end_line)
    let start = offset.unwrap_or(1).max(1).saturating_sub(1); // Convert 1-indexed to 0-indexed
    let line_limit = limit.unwrap_or(MAX_READ_LINES).min(MAX_READ_LINES);

    let end = if let Some(el) = end_line {
        let el_idx = el.saturating_sub(1); // Convert 1-indexed to 0-indexed
        if el_idx < start {
            start + 1 // At least one line
        } else {
            (el_idx + 1).min(start + line_limit)
        }
    } else {
        start + line_limit
    };
